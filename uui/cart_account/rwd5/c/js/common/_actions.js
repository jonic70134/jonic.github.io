...
//bouns list
    $('.bonus-more').on('click', function(){        
        ...
        if (b_detail.css('display') == 'block') {
            b_detail.slideUp(300);
            arrow.removeClass("fa-caret-down");
            arrow.addClass("fa-caret-right");
        }else if (b_detail.css('display') == 'none') {
...
//天的下拉效果
    $('.link-more-icon').on('click', function() {
        //$('.link-more-user').fadeIn();
        $('.link-more-user').animate({marginTop:'5px',opacity:'1'}).css('visibility','visible');
    });
    $('.link-more-close').on('click', function() {
        //$('.link-more-user').fadeOut();
        $('.link-more-user').animate({marginTop:'',opacity:'0'},200).css('visibility','hidden');
    });
