...
function lists_process(data)
{
    ...
    if(items['check']['status'] == '1' || items['check']['status'] == '2')
    {
        ...
        if(action == 'enterData' || action == 'confirm' )
        {
    ...
    $.each(items['contents'] , function(key , value){
        ...
        if(items['contents'][key]['options']['master_id'] == '')
        {
            ...
            if(items['contents'][key]['options']['k6_s'] == '1')
            {
                ...
                '<div class="column1 col888fotb ' + p_class + '"><a class="col888B" href="' + items['contents'][key]['options']['source_url'] + '" target="_blank">' + items['contents'][key]['options']['sell_name'] + ', includes：</a></div>' + //combo
            ...
            else
            {
                ...
                else
                {
                    ...
                    var add_item= 'Add On Shopping Cart - ';
                }
                    ...
                    for(var makey=0;makey<=ma_content.length-1;makey++)
                    {
                        if(ma_content[makey]['camp_seq'] != null && kind==1 && ma_content[makey]['camp_type']!= null)
                        {
                            if(ma_content[makey]['reach'] != 1)                            //是否達成活動門檻(1:是 0:否)
                            {
                                ...
                                if(action == "cartlist")
                                {
                                    ma_name = ma_name + '<div class="listr actlist clearfix"><div class="column0"></div><div class="shopok"><span class="shopadtxt">Buy ' + sign + ma_content[makey]['lacks'] + ' more to get </span> ' + ma_url + '</div></div>';
                                }
                            }
                            else
                            {
                                ...
                                ma_name =  ma_name + '<div class="listr actlist clearfix"><div class="column0"></div><div class="shopok"><span class="shopoktxt">Qualified</span> ' + ma_url + '</div></div>';
        ...
        if(t_num==(content_leng))
        {
            ...
            for(var makey2 in ma_name_kind2)
            {
                ...
                var date_forma_month = months[date_forma_ori.getMonth()];
                var date_forma_day = date_forma_ori.getDate();
                var date_forma_full = date_forma_month + ' ' + date_forma_day + ', ' + date_forma_year;
                ma_name2 = ma_name2 + '<div class="talactok">' +
                                        '<p><span class="shopoktxt">Qualified</span><span class="acttxt">' + ma_name_kind2[makey2].camp_name + ', you will receive shopping credits </span><span class="colred"><span class="actunm">' + ma_name_kind2[makey2].discount + '.</span></span></p>' +
                                        '<p class="actexpl">The shopping credits will be credited to your account on ' + date_forma_full + '. Any return items before ' + date_forma_full + ' may result in voidance of the shopping credits.</p>' +
                                      '</div>';
            }
            $('#act_conform').html(ma_name2);
        }
    ...
    // do not include fee.
    if(action == 'cartlist' || action == 'enterData')
    {
        $('#fee_list').hide();
        if(items['fee']['status'] == 'Y')
        {
            fee_total = (items['fee']['price_show'].replace(/[,]+/g, ''));

        }

        var cart_total_val = ((items['cart_total']).replace(/[,]+/g, ''));

        var show_price = cart_total_val - fee_total;
        $('#total_price, span.pricenum').text(commafy(number_format(show_price, items['decimal'])));
    ...
    if (items['ship_type'])
    {
        var oelem = $('input:radio#shipping_' + items['ship_type']).prop('checked', true);
        if (oelem)
        {
            var elems = $("input:radio[name='" + oelem.attr("name") + "']");
            for (var i=0; i<elems.length; i++)
            {
                var oeq = elems.eq(i);
                $("#" + oeq.data("shipping")).text((oeq.val() == oelem.val()) ? (items["sign"] + oeq.data("carriage")) : "");
                //console.log((oeq.val() == oelem.val()) ? "Y" : "N");
            }
    ...
    if(items['btn_8'] == 'N')
    {
        $('#btn_8').addClass('submitdisp');
        $('#btn_8').attr('disabled' , true);
    }
    else
    {
        $('#btn_8').removeClass('submitdisp');
        $('#btn_8').attr('disabled' , false);
    }
...
function do_point_process(data)
{
    ...
    if(action == 'cartlist' || action == 'enterData')
    {
        var fee = items['source_total'] - items['source_total_no_fee'];
        var total = (items['total_show'].replace(/[,]+/g, ''));
        var show_price = total - fee;
        $('#total_price').html(commafy(number_format(show_price, items['decimal'])));
    }
    else
    {
