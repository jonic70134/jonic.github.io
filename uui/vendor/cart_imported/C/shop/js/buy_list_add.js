...
function lists_process_2(data2)
{
    ...
    {
        if(data[i].IT_MPRICE.length < 10 && data[i].IT_MPRICE != '0')
        {
        ...
        {
            data[i].SALEQTY = '<button id="cartadd_url_'+ i + '" class="btn-buy"  onmouseover="eneter=false" onclick="go_setdata2(\''+data[i].SM_SEQ+'\',\'' + data[i].SET_SDATA +'\', \'cartadd'+ i + '\')">Add to Cart</button><p id="button_msg' + i +'" class="sold-out" style="display:none">-Out of Stock</p>';
        }
        else
        {
            data[i].SALEQTY = '<p class="sold-out">-Out of Stock</p>';
        }
    ...
    $.ajax({
        ...
        success: function(msg) {
            ...
            if(!items.status)
            {
                ...
                {
                    alert('Please add one item in the cart first.');
                }
                else if(items.doc !='OK')
                {
                    alert('The sale-able Qty of this item is zero. Please choose other items.');
                }
            }
            get_lists(bccode);
        } ,
